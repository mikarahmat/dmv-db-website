<?php
        ...
        if(isset($_POST['update'])){
            ...
            // update user data
            $result = mysqli_query($conn, "UPDATE registration SET plate='$plate', registrationDate='$rdate', expiryDate='$edate' WHERE registrationID=$id");
        ...
        $result = mysqli_query($conn, "SELECT * FROM registration WHERE registrationID=$id");

        while($registration = mysqli_fetch_array($result)) {
            $plate = $registration['plate'];
            $rdate = $registration['registrationDate'];
            $edate = $registration['expiryDate'];
        }
  ...
  <section id="portfolio-details" class="portfolio-details">
    <div class="container">
      ...
      <div class="row gy-4">
        ...
        <form name="update" method="post" action="a-registrationedit.php">
        <div class="form-group">
                                <label for="plate">Plate:</label>
                                <input type="text" name="plate" value="<?php echo htmlspecialchars($plate); ?>">
                            </div>
                            <div class="form-group">
                                <label for="registrationDate">Registration Date:</label>
                                <input type="date" name="registrationDate" value=<?php echo htmlspecialchars($rdate); ?>>
                            </div>
                            <div class="form-group">
                                <label for="expiryDate">Expiry Date:</label>
                                <input type="date" name="expiryDate" value=<?php echo htmlspecialchars($edate);?>>
                            </div>
                            <input type="hidden" name="ID" value=<?php echo $_GET['ID'];?>>
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary" name="update" value="update" >Update</button>
                                <button type="button" class="btn btn-default" onclick="location.href='registrationAdmin.php'">Cancel</button>
                            </div>
    </form>
    </div>
        </div>
